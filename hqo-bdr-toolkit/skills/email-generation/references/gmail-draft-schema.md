...
After email generation, the `/hqo:push-drafts` command first saves all emails to a JSON review file, then creates Gmail drafts directly using Cowork's built-in Gmail MCP connector. No webhooks, no API keys, no external tools.

## Flow

1. Emails generated by `/hqo:outreach` are collected
...
- No emails generated → Prompt BDR to run `/hqo:outreach [company]` first
